...
허용되지 않은 상태 변경 요청이 들어오면 일관된 예외 응답을 반환합니다.
...
동시에 여러 주문이 같은 상품의 재고를 차감할 수 있으므로, 재고 차감 대상 상품을 조회할 때 락을 사용합니다. 기본 구현 방향은 상품 row에 비관적 락을 적용하는 방식입니다.

```text
상품 조회 -> 재고 검증 -> 재고 차감 -> 주문 상태 변경
```
